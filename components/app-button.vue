<template>
  <span class="relative font-sans group">
    <span v-if="$slots.tooltip" class="group-hover:opacity-100 opacity-0 pointer-events-none transition-all rounded-xs px-3 py-2 bg-black text-white absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-[calc(100%+8px)] text-xs before:content-[''] before:absolute before:-translate-x-1/2 before:left-1/2 before:top-0 before:-translate-y-full before:border-4 before:border-transparent before:border-b-black text-nowrap">
      <slot name="tooltip" />
    </span>
    <a :href="href" target="_blank" class="inline-flex flex-1 md:flex-0 items-center justify-center gap-2 rounded-md text-xs h-10 min-w-10 border-black border-2 text-black transition-all font-sans font-black hover:shadow-lg active:shadow-none" :class="$slots.default ? 'px-4' : 'px-0'">
      <span v-if="$slots.default" class="-mb-[2px]">
        <slot />
      </span>
      <span class="[&_svg]:h-6">
        ...
        <span v-else-if="href.startsWith('https://note.com')">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 493 493"><path d="m139.57,142.06c41.19,0,97.6-2.09,138.1-1.04,54.34,1.39,74.76,25.06,75.45,83.53.69,33.06,0,127.73,0,127.73h-58.79c0-82.83.35-96.5,0-122.6-.69-22.97-7.25-33.92-24.9-36.01-18.69-2.09-71.07-.35-71.07-.35v158.96h-58.79v-210.22Z"/></svg>
        </span>
        <span v-else>
